...
def tokenize_and_build_vocab(texts):
    ...
    vocab['<PAD>'] = 0
    vocab['<UNK>'] = 1
    return tokens_list, {word: idx + 2 for idx, (word, _) in enumerate(vocab.items())}
...
model = LSTMModel(vocab_size, 100, 128, output_dim)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=0.005)

def interactive_training(model, criterion, optimizer, user_query, user_response=None, correct=True):
    global output_dim
    ...
    if correct:
        ...
        with torch.no_grad():
            ...
            if feedback == 'no':
                ...
                model.train()
                interactive_training(model, criterion, optimizer, user_query, correct_response, False)
        return
    else:
        model.train()
        if user_response not in response_to_idx:
            ...
            output_dim += 1
            model.expand_output_layer(output_dim)
            optimizer = optim.Adam(model.parameters(), lr=0.005)
        ...
        if output_dim % 10 == 0:
            torch.save(model.state_dict(), f"model_epoch_{output_dim}.pth")

while True:
    user_query = input("\nEnter your query (or 'exit' to stop): ").strip()
    if user_query.lower() == 'exit':
        break
    interactive_training(model, criterion, optimizer, user_query)
